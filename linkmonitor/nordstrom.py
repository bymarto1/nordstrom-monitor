...
def get_title(sc):
    return re.search('page_name": "(.+?)"',sc).group(1).strip()
    

def get_image(sc):
    return re.search('product_image_url": "(.+?)"',sc).group(1).strip()
...
class Monitor:
	...
	async def process_url(self, url, proxy):
		...
		if 'Sold Out'  in response.text_content:
			print( url , 'OOS')
			instock = False
		else:
			instock = True
		...
		sizes=[]
		if instock:
			sizechart = re.search('rwd-variation-select"(.+?)</div>',response.text_content, re.S).group(1).strip()
			#print(sizechart)

			sizesDirty = re.findall( '<li>(.+?)</li>',sizechart, re.S)
			for sizeD in sizesDirty:
				size= re.search('SIZE">(.+?)SIZE',sizeD).group(1).strip()
				sizes.append(size)
